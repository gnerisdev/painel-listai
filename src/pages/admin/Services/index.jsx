...
const Services = () => {
  ...
  const formatQuantity = (value) => value.replace(/\D/g, '').slice(0, 4);

  const openEditModal = (item) => {
    setCurrentService({ ...item, active: item.active ? 'true' : 'false' });
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (currentService.id) {
        ...
        // setTableData(prev => prev.map(item => item.id === currentService.id ? response.data.data : item));
      } else {
  ...
  const handleDelete = async (item) => {
    const confirm = window.confirm(
      `Deseja realmente deletar o serviço "${item.title}"?`,
    );
    if (confirm) {
      try {
      ...
      <S.Main>
        ...
        <Table
          data={tableData.map((item) => ({
            ...item,
            activeText: item.active ? 'Ativo' : 'Desativado',
          }))}
          columns={[
          ...
          actions={[
            ...
            { label: 'Editar informações', onClick: openEditModal },
          ]}
        />
        ...
        >
          ...
          <S.WrapperForm>
            ...
            <Select
              ...
              onChange={(value) => setCurrentService({ ...currentService, active: value === 'true' })}
            />
